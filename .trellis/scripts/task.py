...
Usage:
    ...
    python task.py worktree <dir> [--dry-run]  # Create task branch/worktree
    python task.py create-pr [dir] [--dry-run] # Create or stage a task PR
    python task.py sync-pr [dir]               # Sync PR metadata/body
    python task.py review-pr [dir]             # Write local PR review artifact
    python task.py finish-pr [dir]             # Mark PR ready for review
...
Usage:
  ...
  python task.py sync-pr [dir]                      Sync PR metadata/body
  python task.py review-pr [dir]                    Write local PR review artifact
  python task.py finish-pr [dir]                    Mark PR ready for human review
  python task.py set-branch <dir> <branch>          Set git branch
  python task.py set-base-branch <dir> <branch>     Set PR target branch
  python task.py set-scope <dir> <scope>            Set scope for PR title
  python task.py archive <task-dir>                 Archive completed task
...
Examples:
  ...
  python task.py worktree <dir> --dry-run
  python task.py create-pr <dir> --draft --dry-run
