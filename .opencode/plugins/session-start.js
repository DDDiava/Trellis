...
function getTaskStatus(ctx, platformInput = null) {
  ...
  if (taskStatus === "completed") {
    return `Status: COMPLETED\nTask: ${taskTitle}\nSource: ${active.source}\nNext: If the PR is not merged yet, sync/review/finish the PR and wait for human review. After merge or explicit local-only completion, run post-merge reconcile from \`.trellis/workflow.md\`; archive and journal only after the local base branch is current.`
  }
