...
function resolveProjectCwd(p: string): string {
  if (isPosixAbsolutePath(p)) return path.posix.normalize(p);
  if (path.win32.isAbsolute(p)) return path.win32.normalize(p);
  return path.resolve(p);
}

function comparableProjectCwd(p: string): string {
  const resolved = resolveProjectCwd(p).replace(/\\/g, "/");
  return isPosixAbsolutePath(p) ? resolved : resolved.toLowerCase();
}
...
export function sameProject(
  ...
  const a = comparableProjectCwd(sessionCwd);
  const b = comparableProjectCwd(target);
...
function claudeProjectDirFromCwd(cwd: string): string {
  // Claude sanitizes path separators, drive colons, and underscores to '-'.
  return path.join(
    CLAUDE_PROJECTS,
    resolveProjectCwd(cwd).replace(/[/\\:_]/g, "-"),
  );
}
