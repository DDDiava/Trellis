...
def _get_task_status(trellis_dir: Path, hook_input: dict) -> str:
    ...
    if task_status == "completed":
        return (
            ...
            "Next: If the PR is not merged yet, sync/review/finish the PR and wait for human review. "
            "After merge or explicit local-only completion, run post-merge reconcile from "
            "`.trellis/workflow.md`; archive and journal only after the local base branch is current."
        )
