...
| Change the wrap-up flow | Update Phase 3 and `finish-work` related descriptions. In PR-first repositories, current split: Phase 3.4 = AI-driven code commits (batched, user-confirmed), Phase 3.5 = PR handoff, Phase 3.7 = post-merge reconcile, Phase 3.8 = archive + journal. `/finish-work` refuses dirty code changes and must not archive before PR merge or explicit local-only completion. |
